...
**Template profile**:
The canonical, user-configured VS Code profile at `~/.vscode-agent-bridge/data/User` (theme, keybindings, snippets, pane geometry). Configured once via an `install.sh`-launched window; copied into each session's fresh `data-<pid>` dir on spawn. `User/settings.json` existing is the configured-marker. Distinct from cline-sr's shared config, which lives under the same parent but is symlinked, never copied (ADR-0072).
_Avoid_: profile template, seed profile, master profile
...
Fresh session data-dirs suppress every first-run interactive prompt (welcome, workspace trust, cline-sr URI-handler trust, Copilot sign-in) via `SEED_SETTINGS`, and inherit the user's editor look-and-feel by copying the Template profile on spawn. `*.vscdb` files are snapshotted with SQLite's online backup API so a live template window can't produce a torn copy; `saoudrizwan.claude-dev` is excluded from the copy because it stays an ADR-0072 symlink. Pane geometry applies only when the session workspace path matches one opened in the template window (VS Code keys it by workspace-path hash) — accepted limitation.
