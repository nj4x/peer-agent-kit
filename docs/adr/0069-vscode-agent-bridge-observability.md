...
**Scope: multi-session now supported** — ADRs 0071, 0072, 0073, and 0075 give each concurrent MCP server process its own PID-scoped VS Code data dir and window, resolving the original single-instance assumption's collision gap. Concurrent sessions no longer contend for one `code --user-data-dir` target. The log file `~/.vscode-agent-bridge/logs/vscode-agent-bridge.log` is still shared across concurrent server processes on one user account; the `[PID:%(process)d]` field in the log format, together with each session's PID-scoped data dir name (`data-<pid>`), lets a reader attribute interleaved log lines to the window that produced them. Process-safe log rotation (e.g., `WatchedFileHandler` or per-PID log files) is not implemented — `RotatingFileHandler`'s size-based rotation is not coordinated across processes — but is no longer a correctness gap for task dispatch, since each session's tasks flow through its own window and port.
