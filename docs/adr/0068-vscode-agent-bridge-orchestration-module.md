...
See [`docs/diagrams/vscode-agent-bridge-e2e.md`](../diagrams/vscode-agent-bridge-e2e.md) for the full set of end-to-end sequence diagrams covering server lifespan, the three MCP tool paths (`ask_peer_agent`, `submit_to_peer_agent`/`poll_peer_agent`, `close_peer_agent`), task execution/hook events, and failure paths.
